...
if [ -z $1 ]; then
  ...
  if [[ ! -z ${description} ]]; then
    dqt='"'
    MR+=" -o merge_request.description=${dqt}/assign @JasonKirshner ${description}${dqt}"
  else
    MR+=' -o merge_request.description="/assign @JasonKirshner"'
  fi
    ...
    if [[ ${!i} == -* ]]; then
      case "${!i}" in
        ...
        -d|-description)
          ...
          if [[ ! -z ${!val} ]] && [[ ${!val} != -* ]]; then
            dqt='"'
            MR+=" -o merge_request.description=${dqt}/assign @JasonKirshner ${!val}${dqt}"
          else
